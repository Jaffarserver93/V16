...
const PaymentForm: React.FC<PaymentFormProps> = ({ selectedPlan, selectedAddons, onBack, theme }) => {
  ...
  const calculateTotal = () => {
    const basePrice = Number.parseInt(selectedPlan.price.replace(/[₹$,]/g, "").split("/")[0])
    const unitsPrice =
      (localAddons?.units || 0) * Number.parseInt(selectedPlan.addons?.unit?.replace(/[₹$,]/g, "") || "0")
    const backupsPrice =
      (localAddons?.backups || 0) * Number.parseInt(selectedPlan.addons?.backup?.replace(/[₹$,]/g, "") || "0")
      ...
      embeds: [
        {
          ...
          fields: [
            ...
            {
              name: "💰 Pricing",
              value: `**Base Price:** $${selectedPlan.price.replace(/[₹$,]/g, "").split("/")[0]}/month\n**Add-ons:** $${(localAddons?.units || 0) * Number.parseInt(selectedPlan.addons?.unit?.replace(/[₹$,]/g, "") || "0") + (localAddons?.backups || 0) * Number.parseInt(selectedPlan.addons?.backup?.replace(/[₹$,]/g, "") || "0")}\n**Total:** $${calculateTotal()}/month`,
              inline: true,
            },
            {
              name: "🎮 Server Details",
      ...
      <div className={`min-h-screen ${themeStyles.bg} flex items-center justify-center p-4`}>
        <div className={`max-w-md w-full ${themeStyles.card} rounded-2xl p-6 sm:p-8 text-center`}>
          ...
          <p className={`${themeStyles.textSecondary} mb-6 text-sm sm:text-base`}>
            Your Minecraft hosting order has been received. Create a ticket on Discord with this ID{" "}
          ...
          <div className={`${themeStyles.card} rounded-2xl p-4 sm:p-6 h-fit order-2 xl:order-1`}>
            ...
            <div className="space-y-4 mb-4 sm:mb-6">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs sm:text-sm">
                ...
                <div className={`flex justify-between ${themeStyles.textSecondary}`}>
                  <span>Location:</span>
                  <span className="font-medium">{selectedPlan.location || "Default"}</span>
                </div>
              </div>
            </div>
            ...
            {selectedPlan.addons && (
              <div
                className={`border-t ${theme === "light" ? "border-gray-200" : "border-white/20"} pt-4 sm:pt-6 mb-4 sm:mb-6`}
              >
                <h4 className={`text-base sm:text-lg font-semibold ${themeStyles.text} mb-3 sm:mb-4`}>
                  Configure Add-ons
                </h4>

                {/* Extra Units */}
                {selectedPlan.addons.unit && (
                  <div className={`${themeStyles.card} p-3 sm:p-4 rounded-lg mb-3 sm:mb-4`}>
                    <div className="flex items-center justify-between mb-2 sm:mb-3">
                      ...
                      <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
                        ...
                        <span
                          className={`w-6 sm:w-8 text-center font-semibold ${themeStyles.text} text-sm sm:text-base`}
                        >
                          {localAddons?.units || 0}
                        </span>
                        <button
                    ...
                    {(localAddons?.units || 0) > 0 && (
                      <div className={`text-right text-xs sm:text-sm ${themeStyles.textSecondary}`}>
                        Subtotal: $
                        {(localAddons?.units || 0) * Number.parseInt(selectedPlan.addons.unit.replace(/[₹$,]/g, ""))}
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div className={`${themeStyles.card} p-3 sm:p-4 rounded-lg`}>
                    ...
                    {(localAddons?.backups || 0) > 0 && (
                      <div className={`text-right text-xs sm:text-sm ${themeStyles.textSecondary}`}>
                        Subtotal: $
                        {(localAddons?.backups || 0) *
                          Number.parseInt(selectedPlan.addons.backup.replace(/[₹$,]/g, ""))}
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="text-right">
                  {appliedCoupon && (
                    <div className={`text-sm ${themeStyles.textMuted} line-through mb-1`}>
                      $
                      {Number.parseInt(selectedPlan.price.replace(/[₹$,]/g, "").split("/")[0]) +
                        (localAddons?.units || 0) *
                          Number.parseInt(selectedPlan.addons?.unit?.replace(/[₹$,]/g, "") || "0") +
                        (localAddons?.backups || 0) *
                          Number.parseInt(selectedPlan.addons?.backup?.replace(/[₹$,]/g, "") || "0")}
                      /mo
                    </div>
                  )}
