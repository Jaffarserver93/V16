...
import { authManager, superDatabase, type Coupon } from "@/utils/database"
...
export default function DomainOrderForm({ domain }: DomainOrderFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const orderDetails = {
      ...
      orderId: `DOMAIN-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`,
      planName: domain.name, // For domains, planName is the domain name itself
    ...
    try {
      ...
      if (discordWebhookUrl) {
        await fetch(discordWebhookUrl, {
          ...
          body: JSON.stringify({
            content: `New Domain Order!
            Order ID: ${orderDetails.orderId}
            Domain: ${orderDetails.planName}
      ...
      toast({
        title: "Order Placed!",
        description: `Your domain order for ${domain.name} has been successfully placed. Order ID: ${orderDetails.orderId}`,
      })
    } catch (error) {
    ...
    return (
      <Card className="bg-white/10 backdrop-blur-md border border-white/20 text-white p-6">
        ...
        <CardContent className="text-center">
          ...
          <p className="text-white/70">Your domain registration for {domain.name} has been successfully placed.</p>
  ...
  return (
    <Card className="bg-white/10 backdrop-blur-md border border-white/20 text-white p-6">
      <CardHeader>
        <CardTitle className="text-2xl font-bold">Register {domain.name}</CardTitle>
        <p className="text-white/70">
          Complete your purchase for the domain <span className="font-semibold text-purple-400">{domain.name}</span>.
        </p>
      </CardHeader>
      <CardContent>
